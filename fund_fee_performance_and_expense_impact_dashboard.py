...
def future_value(starting_value, annual_contribution, gross_return, expense_ratio, years):
...
page = st.sidebar.radio("Navigate", ["Fees & Expenses", "Performance Comparison", "Expense Ratio Impact"])
...
if page == "Fees & Expenses":
    ...
    st.bar_chart(available.set_index("Ticker")[["Return"]])

    st.subheader("Multi-period comparison")
    selected_tickers = st.multiselect("Select funds to compare", options=available["Ticker"].tolist(), default=available["Ticker"].tolist()[: min(6, len(available))])
...
""")
    ...
    st.header("Expense Ratio Impact")
    st.write("This illustration estimates how an annual expense ratio can reduce a long-term portfolio's ending value. It assumes an 8.00% annual gross return before expenses, a $10,000 starting value, $50,000 contributed at the end of each year, and a 30-year investment period.")

    expense_ratio_percent = st.slider(
        "Expense ratio (%)",
        min_value=0.00,
        max_value=2.00,
        value=0.50,
        step=0.01,
        format="%.2f%%",
        help="Move the slider to change the annual expense ratio used in the illustration.",
    )
    ...
    gross_return = 0.08
    years = 30
    expense_ratio = expense_ratio_percent / 100
    net_return = gross_return - expense_ratio
    final_value = future_value(starting_value, annual_contribution, gross_return, expense_ratio, years)
    no_fee_value = future_value(starting_value, annual_contribution, gross_return, 0.0, years)
    fee_impact = no_fee_value - final_value
    total_contributions = starting_value + annual_contribution * years

    col1, col2, col3 = st.columns(3)
    col1.metric("Final investment value", f"${final_value:,.0f}")
    col2.metric("Net annual return after expenses", f"{net_return:.2%}")
    col3.metric("Difference versus 0.00% expenses", f"${fee_impact:,.0f}")

    st.info(f"With a {expense_ratio_percent:.2f}% annual expense ratio, the assumed 8.00% gross return becomes a {net_return:.2%} net annual return. Total invested capital is ${total_contributions:,.0f} under the stated contribution timing.")

    st.subheader("How changing the expense ratio affects the ending value")
    scale = pd.DataFrame({"Expense Ratio (%)": [round(x / 100, 2) for x in range(0, 201, 10)]})
    scale["Final Value"] = scale["Expense Ratio (%)"].map(lambda x: future_value(starting_value, annual_contribution, gross_return, x / 100, years))
    scale["Value Lost vs. 0.00%"] = no_fee_value - scale["Final Value"]

    chart_scale = scale.set_index("Expense Ratio (%)")[["Final Value"]]
    st.line_chart(chart_scale)

    display_scale = scale.copy()
    display_scale["Expense Ratio (%)"] = display_scale["Expense Ratio (%)"].map("{:.2f}%".format)
    display_scale["Final Value"] = display_scale["Final Value"].map("${:,.0f}".format)
    display_scale["Value Lost vs. 0.00%"] = display_scale["Value Lost vs. 0.00%"].map("${:,.0f}".format)
    st.dataframe(display_scale, use_container_width=True, hide_index=True)

    with st.expander("Calculation assumptions"):
        st.markdown("""
- The initial $10,000 is invested at the beginning of the illustration.
- The $50,000 annual contribution is assumed to be made at the end of each year for 30 years.
- The assumed 8.00% gross annual return and the selected expense ratio are constant every year.
- The expense ratio is modeled as a direct reduction in annual return: net return = 8.00% minus the selected expense ratio.
- The illustration excludes taxes, inflation, trading costs, plan fees, changes in contribution amounts, fund distributions, and variation in investment returns.
- This is a hypothetical mathematical illustration, not a forecast or investment recommendation.
""")
